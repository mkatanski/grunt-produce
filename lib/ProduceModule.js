...
var ProduceModule = (function () {
  ...
  ProduceModule.prototype.validate = function () {
    _.each(this.locals.variables, function (value, varName) {
      ...
      if (!_.isFunction(value) && _variable.required === true) {
        if (_.isNaN(value) || _.isNull(value) || value.trim() === '') {
          throw 'Variable [' + varName + '] is required';
        }
      }

      // validate variable
      if (_.isFunction(_variable.validate)) {
        if(!_.isFunction(value) && !_variable.validate(value, validator)) {
          throw  varName + ' value is incorrect';
        }
      }
    }, this);
  };
